...
; Exercise 2.15
; The above discussion is related to why Eva Lu Ator says that interval arithmetic computations
; will produce tighter error bounds when using expressions written in a form such that no
; variable that represents an uncertain number is repeated.
; Repeating variables that represent uncertain numbers adds additional and unncessary
; uncertainty to any expressions that involve multiplication or division.
; This is why par2 is a "better" program for parallel resistors than par1.

; Exercise 2.16
; In general, equivalent algebraic expressions may lead to different answers with interval
; arithmetic because interval arithmetic does not follow the usual laws of number
; arithmetic and algebra that we are used to.
; For further discussion, see the page for this exercise on the Scheme Wiki
; http://community.schemewiki.org/?sicp-ex-2.14-2.15-2.16
; or this discussion from Drew Hess
